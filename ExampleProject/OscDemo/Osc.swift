...
class Osc {
    ...
    func SetHost(host: String, callback: () -> Void) {
        socketReady_ = false;
        host_ = host
        if sock_ > 0 {
            close(sock_)
    ...
    func SetPort(port: Int) {
        ...
        if sa_family_ == sa_family_t(AF_INET) {
            ...
            addr4_.sin_len = UInt8(sizeofValue(addr4_))
    ...
    func GetHostByName(hostname: String) {
        ...
        let hostref = CFHostCreateWithName(nil, hostname)
        ...
        CFHostSetClient(host, { (host, infoType, error, info) in
            ...
            let obj = unsafeBitCast(info, Osc.self)
            obj.GetAddress(host)
    ...
    func GetAddress(host:CFHost) {
        ...
        for i in 0 ..< CFArrayGetCount(address_list_) {
            ...
            addr4_ = unsafeBitCast(address4.memory, sockaddr_in.self)
        ...
        if (sa_family_ == UInt8(AF_INET)) {
            withUnsafePointer(&addr4_) { ptr -> Void in
                ...
                sendto(sock_, sendbuf_, sendbuf_.count, 0,
                    addrptr, socklen_t(sizeof(sockaddr_in)))
            }
        } else {
            withUnsafePointer(&addr6_) { ptr -> Void in
                let addrptr = UnsafePointer<sockaddr>(ptr)
                sendto(sock_, sendbuf_, sendbuf_.count, 0,
                    addrptr, socklen_t(sizeof(sockaddr_in6)))
            }
        }
    }
    ...
    func PushArg(arg: Int) {
        ...
        databuf_ += toByteArray(argI32.bigEndian)
    ...
    func PushArg(arg: Float) {
        let i:UInt32 = unsafeBitCast(Float32(arg), UInt32.self)
        databuf_ += toByteArray(i.bigEndian)
